...
const Team = (Props) => {
    ...
    const getAll = () => {
        TeamService.getPaginatedAll({ maxResultCount: pagination.pageSize, skipCount: (pagination.current - 1) * pagination.pageSize, name: filter }).then(res => {
            if (res) {
    ...
    return (
        <Card>
            <div style={{ display: 'flex', justifyContent: 'space-between', margin: '10px' }}>
                <h1>Manage Teams</h1> <Button type="primary" shape="round" icon="plus" onClick={() => setIsOpenModal(true)} >Add</Button>
            </div>

            <Table pagination={pagination} columns={columns} dataSource={teamList} scroll={{ x: 1500, y : 1000 }} onChange={handleTableChange} />

            <CustomModal title="Create Team" isModalVisible={isOpenModal} handleCancel={() => { setIsOpenModal(false) }} handleSubmit={teamFormHandler} >
                <Form onSubmit={teamFormik.handleSubmit}>
                    <CustomInput title="Name" type="text" handleChange={handleChange} value={teamFormik.values.name} stateKey="name" placeholder="" errorMessage={teamFormik.errors.name} />
                    <CustomInput title="Place" type="text" handleChange={handleChange} value={teamFormik.values.place} stateKey="place" placeholder="" />
                    <CustomInput title="Zone" type="number" handleChange={handleChange} value={teamFormik.values.zone} stateKey="zone" placeholder="" />
                    <CustomInput title="Contact" type="number" handleChange={handleChange} value={teamFormik.values.contact} stateKey="contact" placeholder="" />
                    ...
                    <CustomInput title="Registered" type="checkbox" handleChange={handleChange} value={teamFormik.values.isRegistered} stateKey="isRegistered" placeholder="" />
                </Form>
            </CustomModal>
        </Card >
    );
