    ...
    return (
        <Card>
            <div style={{ display: 'flex', justifyContent: 'space-between', margin: '10px' }}>
                <h1>Manage Matches</h1> <Button type="primary" shape="round" icon="plus" onClick={() => setIsOpenModal(true)} >Add</Button>
            </div>
            <Table columns={columns} dataSource={matchList} scroll={{ x: 1500, y: 1000 }} />

            <CustomModal title="Add a new Match" isModalVisible={isOpenModal} handleCancel={() => { setIsOpenModal(false) }} handleSubmit={handleSubmit} >
                <Form>
                    <CustomInput title="Team 1" type="select" options={teamList} handleChange={handleChange} value={matchFormik.values.team1.name} stateKey="team1" placeholder="" />
                    <CustomInput title="Team 2" type="select" options={teamList} handleChange={handleChange} value={matchFormik.values.team1.name} stateKey="team2" placeholder="" />
